...
class TaylorMergePruner(MetaPruner):
    def set_storage(self, num_type='many'):
        if num_type == 'many':
            self.groups_imps_many = {}
            self.counter_many = {}
        elif num_type == 'mid':
            self.groups_imps_mid = {}
            self.counter_mid = {}
        elif num_type == 'few':
            self.groups_imps_few = {}
            self.counter_few = {}
        else:
            assert 0, 'unsupported type'

    def store_importance(self, num_type='many'):
        ...
        for i, group in enumerate(self.DG.get_all_groups(ignored_layers=self.ignored_layers, root_module_types=self.root_module_types)):
            if self._check_pruning_ratio(group):  # check pruning ratio
                ...
                # print(imp)
                if num_type == 'many':
                    if i not in self.groups_imps_many:
                        self.groups_imps_many[i] = imp
                    else:
                        self.groups_imps_many[i] += imp
                    if i not in self.counter_many:
                        self.counter_many[i] = 1
                    else:
                        self.counter_many[i] += 1
                elif num_type == 'mid':
                    if i not in self.groups_imps_mid:
                        self.groups_imps_mid[i] = imp
                    else:
                        self.groups_imps_mid[i] += imp
                    if i not in self.counter_mid:
                        self.counter_mid[i] = 1
                    else:
                        self.counter_mid[i] += 1
                elif num_type == 'few':
                    if i not in self.groups_imps_few:
                        self.groups_imps_few[i] = imp
                    else:
                        self.groups_imps_few[i] += imp
                    if i not in self.counter_few:
                        self.counter_few[i] = 1
                    else:
                        self.counter_few[i] += 1
                else:
                    assert 0, 'unsupported type'
    ...
    def prune_local(self, num_type='many') -> typing.Generator:
        ...
        for i, group in enumerate(self.DG.get_all_groups(ignored_layers=self.ignored_layers,
            ...
            if self._check_pruning_ratio(group):  # check pruning ratio
                ...
                ch_groups = self._get_channel_groups(group)
                if num_type == 'many':
                    imp = self.groups_imps_many[i] / self.counter_many[i]
                elif num_type == 'mid':
                    imp = self.groups_imps_mid[i] / self.counter_mid[i]
                elif num_type == 'few':
                    imp = self.groups_imps_few[i] / self.counter_few[i]
                else:
                    assert 0, 'unsupported type'
    ...
    def prune_global(self, num_type='many') -> typing.Generator:
        # if self.current_step > self.iterative_steps:
        #     warnings.warn("Pruning exceed the maximum iterative steps, no pruning will be performed.")
        #     return
